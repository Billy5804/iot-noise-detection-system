...
public interface SiteUserRepository extends CrudRepository<SiteUser, SiteUserPK> {
	...
	default Iterable<SiteUserOnlyUserIdProjection> deleteUnauthorisedBySiteId(UUID siteId) {
		final Iterable<SiteUserOnlyUserIdProjection> usersToBeDeletedIds = findBySiteUserPKSiteIdAndRole(siteId,
				SiteUserRole.UNAUTHORISED);
	...
	default boolean existsByIdAndAuthorised(SiteUserPK siteUserPK) {
		return existsBySiteUserPKAndRoleNot(siteUserPK, SiteUserRole.UNAUTHORISED);
	}
}
